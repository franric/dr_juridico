      ...
      <div class="col-12">
            <div class="card card-primary">
                <div class="card-header">
                  <h3 class="card-title"><i class="fa fa-dollar-sign"></i>&nbsp; Contas a Receber</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                ...
                <tbody>
                    @if(isset($contasReceber))
                            @foreach($contasReceber as $aReceber)
                                <tr>
                                    <td>{{ $aReceber->numContrato }}</td>
                                    <td>
                                        @foreach($aReceber->Pessoa as $pessoa)
                                            <p class="text-left">
                                                <span>{{ $pessoa->nomeRazaoSocial }}</span>
                                            </p>
                                        @endforeach
                                    </td>
                                    <td> R$ {{ number_format($aReceber->valorContrato, 2, ',', '.') }}</td>
                                    <td class="text-success text-right text-bold">R$ {{ number_format($aReceber->valorPago, 2, ',', '.') }}</td>

                                    <td class="text-danger text-right text-bold">R$ {{ number_format($aReceber->valorReceber, 2, ',', '.') }}</td>
                                    <td class="text-center">{{ $aReceber->numParcelaContrato }}</td>


                                    <td style="text-align: center">
                                        <div style="margin-right:5px" class="btn-group">
                                            <a href="{{ route('parcela', $aReceber->id) }}"
